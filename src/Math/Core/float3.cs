...
namespace Fusee.Math.Core
{
    ...
    public struct float3 : IEquatable<float3>
    {
        ...
        public float this[int idx]
        {
            get
            {
                return idx switch
                {
                    ...
                    1 => y,
                    2 => z,
                    _ => ThrowHelper.ThrowArgumentOutOfRangeException<float>($"Index {idx} not eligible for a float3 type"),
                };
            }
            set
            {
                switch (idx)
                {
                    ...
                    default:
                        ThrowHelper.ThrowArgumentOutOfRangeException($"Index {idx} not eligible for a float3 type");
                        break;
                }
            }
        }
        ...
        public static float3 Parse(string source, IFormatProvider? provider = null)
        {
            ...
            if (strings.Length != 3)
                ThrowHelper.ThrowFormatException("String parse for float3 did not result in exactly 3 items.");
