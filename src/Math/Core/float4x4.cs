    ...
    public struct float4x4 : IEquatable<float4x4>
    {
        ...
        public float this[int i, int j]
        {
            get
            {
                return i switch
                {
                    ...
                    2 => Row3[j],
                    3 => Row4[j],
                    _ => ThrowHelper.ThrowArgumentOutOfRangeException<float>($"Index {i},{j} not eligible for a float4x4 type"),
                };
            }
            set
            {
                switch (i)
                {
                    ...
                    default:
                        ThrowHelper.ThrowArgumentOutOfRangeException($"Index {i},{j} not eligible for a float4x4 type");
                        break;
                }
            }
        }
        ...
        public static float4x4 Parse(string source, IFormatProvider? provider = null)
        {
            ...
            if (strings.Length != 16)
                ThrowHelper.ThrowFormatException("String parse for float4x4 did not result in exactly 16 items.");
